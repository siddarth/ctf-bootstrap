...
#--{{{
  ...
  def check_call(cmd, opts={}, &b)
    ...
    stdout_txt = stderr_txt = nil
    status = popen(cmd, opts) do |pid, stdin, stdout, stderr|
      if b
        b.call(pid, stdin, stdout, stderr)
      else
        stdout_txt = stdout.read if opts[:stdout] == Subprocess::PIPE
        stderr_txt = stderr.read if opts[:stderr] == Subprocess::PIPE
      end
    end
  ...
  def run(*cmd)
    out, err = nil
    status = self.popen(cmd.flatten,
               :stdout => Subprocess::PIPE,
               :stderr => Subprocess::PIPE) do |pid, i, o, e|
      out = o.read
      err = e.read
    end
